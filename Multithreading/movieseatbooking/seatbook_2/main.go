...
// FOR UPDATE Strategy - Pessimistic locking
func bookSeat(db *sql.DB, userID int) BookingResult {
	...
	// Find and lock an available seat
	var seatID int
	var seatName string
	var movieID int
...
func main() {
	log.Println("🎬 Movie Seat Booking - FOR UPDATE Strategy Demo")
	log.Println("================================================")
	...
	// Run the FOR UPDATE test
	runBookingTest(db, 200)

	log.Println("\nKey Takeaways:")
	log.Println("• FOR UPDATE: Prevents race conditions but may cause deadlocks")
	log.Println("• All transactions wait for locks to be released")
	log.Println("• Consistent results but slower performance")
}
